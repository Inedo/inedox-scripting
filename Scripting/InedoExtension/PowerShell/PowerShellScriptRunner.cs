...
internal sealed partial class PowerShellScriptRunner : ILogger, IDisposable
{
    ...
    [GeneratedRegex(@"^\[type::(?<1>[^\]]+)\](?<2>.+)$", RegexOptions.ExplicitCapture | RegexOptions.Compiled)]
    private static partial Regex TypeCastRegex();
    [GeneratedRegex(@"(?>\$(?<1>[a-zA-Z0-9_]+)|\${(?<2>[^}]+)})", RegexOptions.ExplicitCapture | RegexOptions.Compiled)]
